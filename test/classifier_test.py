...
class KoalaClassifierTest(unittest.TestCase):
    ...
    """
    mock_koala_response = [{"label": "OK", "score": 0.7044275999069214}, {"label": "SH", "score": 0.09657368808984756},
                            {"label": "H", "score": 0.08526786416769028}, {"label": "V", "score": 0.038328953087329865},
                            {"label": "HR", "score": 0.024068444967269897},
                            {"label": "S", "score": 0.01646522432565689},
                            {"label": "V2", "score": 0.016412340104579926},
                            {"label": "H2", "score": 0.010275991633534431},
                            {"label": "S3", "score": 0.008179856464266777}]

    @patch('classifier.query_http_endpoint')
    def test_classify(self, mock_query_http_endpoint: Mock):
        mock_query_http_endpoint.return_value = [self.mock_koala_response]
        koala_classifier: KoalaClassifier = KoalaClassifier(
            url='http://dummy.bar',
            bearer_token='foo')

        classification_response: List[KoalaTextModerationLabel] = koala_classifier.classify(
            "This is test message")

        self.assertEqual(self.mock_koala_response, classification_response)

    @patch('classifier.query_http_endpoint')
    def test_error(self, mock_query_http_endpoint: Mock):
        with self.assertRaises(Exception):
            mock_query_http_endpoint.side_effect = Exception('Mock exception')
            koala_classifier: KoalaClassifier = KoalaClassifier(
                url='http://dummy.bar',
                bearer_token='foo')
